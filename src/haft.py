...
def reconstruct_radial_filter_vectorized(profile_1d, patch_size, batch_size):
    ...
    filter_2d = profile_1d[:, d_indices.view(-1)].view(total_patches, H, W).unsqueeze(1)
...
class HAFT(nn.Module):
    ...
    def _compute_hierarchical_contexts_batch(self, channel_features):
        ...
        for level in range(self.num_haft_levels):
            ...
            cv_all = cv_all.view(B, total_patches, C, 2 * self.context_vector_dim)
            cv_all = cv_all.permute(0, 2, 3, 1)  # (B, C, 2*context_dim, total_patches)
            cv_all = cv_all.view(B, C, 2 * self.context_vector_dim, num_patches_h, num_patches_w)
    ...
    def _apply_frequency_filtering_batch(self, channel_features, hierarchical_contexts):
        ...
        mid_dim = enriched_contexts.shape[2] // 2
        ...
        mag_profiles, phase_profiles = self.filter_predictor(
            fused_mag_vectors.view(-1, fused_mag_vectors.shape[2]), 
            fused_phase_vectors.view(-1, fused_phase_vectors.shape[2])
        )
        mag_profiles = mag_profiles.view(B, num_patches_h * num_patches_w * C, -1)
        phase_profiles = phase_profiles.view(B, num_patches_h * num_patches_w * C, -1)
        ...
        enhanced_patches = enhanced_patches.view(B, num_patches_h, num_patches_w, C, patch_h, patch_w)
        enhanced_patches = enhanced_patches.permute(0, 3, 1, 4, 2, 5).contiguous()
        enhanced_patches = enhanced_patches.view(B, C, H, W)
    ...
    def _gather_ancestral_contexts_batch(self, hierarchical_contexts, B, C, num_patches_h, num_patches_w):
        ...
        for level in range(len(hierarchical_contexts)):
            ...
            level_contexts_flat = level_contexts.view(B, C, 2 * self.context_vector_dim, -1)
